			  ...
			  char const *thing, uid_t expected, uid_t got)
{
	...
	simple_error:
		fr_strerror_printf("Semaphore on \"%s\" ID %x - %s is incorrect",
				   file, proj_id, thing);
		return false;
	}
	...
	fr_strerror_printf("Semaphore on \"%s\" ID %x - %s is incorrect.  Expected \"%s\", got \"%s\"",
...
int fr_sem_get(char const *file, int proj_id, bool check_perm)
{
	...
	if (sem_key < 0) {
		fr_strerror_printf("Failed associating semaphore with \"%s\" ID %x: %s",
	...
	if (sem_id < 0) {
		if (errno != ENOENT) {	/* Semaphore existed but we ran into an error */
			fr_strerror_printf("Failed getting semaphore on \"%s\" ID %x: %s",
					   file, proj_id, fr_syserror(errno));
			return -2;
		}
		...
		if (sem_id < 0) {
			fr_strerror_printf("Failed creating semaphore on \"%s\" ID %x: %s",
					   file, proj_id, fr_syserror(errno));
			return -3;
		}
	/*
	 ...
	 */
		...
		ret = semctl(sem_id, 0, IPC_STAT, &info);
		if (ret < 0) {
			fr_strerror_printf("Failed getting semaphore permissions on \"%s\" ID %x: %s",
					   file, proj_id, fr_syserror(errno));
			return -2;
		}

		if (info.sem_perm.mode & S_IWOTH) {
			fr_strerror_printf("Semaphore on \"%s\" ID %x is world writable (insecure)",
					   file, proj_id, fr_syserror(errno));
			return -2;
		}
